...
export function Sidebar({ children, side = "left", sideWidth = "basis-80", space = "gap-0", noStretch = false }: SidebarProps) {
    const sidebarClasses = side === "left"
    ...
    const contentClasses = side == "left"
    ...
    return (
        <div className={`flex flex-wrap ${space} ${sidebarClasses} ${contentClasses} ${stretchClasses}`}>
            {children}
        </div>
    );
}